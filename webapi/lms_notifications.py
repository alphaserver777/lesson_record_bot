...
def _message(payload: dict) -> tuple[str, str]:
    ...
    if event_type == "assignment.review_digest":
        lines = [f"<b>Работы на проверке: {int(payload.get('count', 0))}</b>"]
        for item in payload.get("items", [])[:20]:
            student = item.get("member_name") or item.get("member") or "Ученик"
            assignment = item.get("assignment_title") or item.get("lesson") or "Задание"
            lines.append(f"• {html.escape(student)} — {html.escape(assignment)}")
...
async def receive_lms_notification(
    ...
    allowed = set(os.getenv("LMS_ALLOWED_SOURCE_IPS", "192.168.50.114").split(","))
