          ...
          <div className="stack analytics-stack">
            {activeTab === 'marketing' ? (
              <>
                ...
                {(() => { const campaign = (marketingMetrics?.rows || []).find(row => String(row.campaign_id) === String(selectedMarketingCampaignId)); return campaign ? <div style={{ order: 2 }}><CampaignDetail key={campaign.campaign_id} campaign={campaign} onSavePeriod={saveCampaignPeriod} onSaveMetrics={(id, metrics) => saveCampaignMetrics(id, metrics.views, metrics.dialogs, metrics.target_actions)} /></div> : null })()}
