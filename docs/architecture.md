...
подтверждает identity и открывает его во встроенном браузере. Прокси сайта
передаёт `/cabinet/api/*` в API; после пересоздания API-контейнера прокси нужно
перезагрузить, чтобы обновить адрес контейнера.
...
коммерческая работа; уроки и платежи связаны с тем же `contact_id`. Цена занятия
и имя пользователя Telegram хранятся также в `contacts`, чтобы их можно было
задать лиду до создания профиля ученика. Имя,
телефон и Telegram ID не должны копироваться в новые сущности.

`external_identities` связывает контакт с постоянными идентификаторами
учебной платформы и тренажёра карточек. Внутренние базы этих приложений не
объединяются с CRM. Имя и фамилия ученика фиксируются при первом завершении
регистрации; после этого изменить их может только администратор CRM.
