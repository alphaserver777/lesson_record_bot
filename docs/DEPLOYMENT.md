...
## Обнаруженная блокировка запуска API, 5 сентября 2026 года

При пересоздании API его стартовая процедура `transactions.init_db()` выполняла
`ALTER TABLE ... ADD COLUMN IF NOT EXISTS` и ожидала блокировку от трёх
соединений `idle in transaction`, открытых от 1 часа 40 минут до более суток.
У всех трёх `backend_xid` был пуст: незавершённых изменений базы не было.
После проверки закрыты только эти три блокирующих соединения.

Увеличение времени ожидания запуска само по себе это не исправляет. Нужно
завершать транзакции чтения в фоновых задачах и вынести изменение схемы из
обычного запуска API. Перед повторным запуском проверять `pg_blocking_pids`
и `pg_stat_activity`; нельзя массово завершать активные транзакции с записью.

Исправление от 5 сентября освобождает task-local соединение после каждого
прохода планировщика. При остановке бот отменяет фоновую задачу и закрывает
подключения к базе. Перезапуск больше не удаляет ожидающие сообщения Telegram.
